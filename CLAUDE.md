...
- **RX DMA per-page VA→PA 拆分**: RxExecutor 和 RscEngine 在提交 DMAWriteJob 前，按 4K 页边界拆分 VA 范围，对每页独立做 MMU 翻译（`PAGE_SIZE - ((va + off) & PAGE_MASK)` 计算页内剩余字节）。这确保跨页 payload 正确映射到不连续物理页。适用于所有 RX 数据路径：WRITE/READ_RESP（单 VA）、SEND（multi-SGE 每 SGE 内部也按页拆）、TCP RSC（coalesced payload 按页拆）。Virtio RX 不适用（使用 descriptor table 中的直接 PA，不经 VA→PA 翻译）
