...
- **Virtio-net**: 北向接口前端，独立于 IB 队列机制。TX path: SplitVirtqPort(pipelined 3-step avail ring DMA via VRING_TAG) → VirtioEngine._run_tx_dispatch(**per-QPN worker pattern**: dispatcher 路由到 per-QPN deque，每个 _tx_worker 独立做 GS grant + pipeline pool + tx_cmd push，无跨 QPN HOL 阻塞) → VirtDMAFetch(**pipelined dispatcher+collector, 32 inflight** frame DMA via VIRT_TX_TAG) → EgressFTE → wire；RX path: wire → IngressFTE → RxLookup(virt_exec_out fork) → VirtioEngine.run_rx(**concurrent dispatcher, 8 inflight**: alloc_rx via VRING_TAG + DMAWriteJob(done_evt)) → RxDMAEngine(frame DMA via RX_EXEC_TAG=9) → host RX buffer → _run_rx_completer(序列化 used-ring write) → VirtioEngine.run_tx_complete(VirtUsedEvent→complete_tx)；TX/RX completers 批量写 used ring via VRING_TAG HIF（batch=16 + coalesce timer 500ns）
...
| SplitVirtqPort | `rnic/qm/vring/virtq_port.py` | **纯 QM 层**（类比 SQM，无 GS 交互）: 吸收原 VringPrefetcher + VringDescCache + VringDispatcher + TX/RX completers 全部逻辑。doorbell → 3-step DMA(avail_idx→coalesced ring_slot(batch×2B, 1 DMA)→per-entry desc_table submit) → 内部 `_tx_queue`；Step B ring 读取合并消除 DDR bank 串行化（16×serial→1 access）；`dequeue_tx()` 返回 simpy Store.get() event；`alloc_rx()` generator（ring walk via VRING_TAG）；`complete_tx/complete_rx` 普通方法（非 generator）——缓存 entries 至 batch，满 `_USED_BATCH_MAX=16` 或 `_USED_COALESCE_NS=500` ns timer 到期时 coalesced flush（2-3 fire-and-forget DMA writes 替代原 2N writes）；`_tx_depth[qpn]` 背压计数器（上限 64/QPN）|
| VirtioEngine | `rnic/te/virtio/virtio_engine.py` | **TE 层 Virtio 独立容器**（peer to MsgEngine）: `_run_tx_dispatch()` — **per-QPN worker pattern (cf. RespExecutor)**: dispatcher 从 VirtqPort 拉取 VirtqDescMsg → 按 QPN 路由到 per-QPN deque → 按需 spawn `_tx_worker(qpn)`；每个 worker 独立做 GS grant 等待 + pipeline pool 扣减 + tx_cmd push（无跨 QPN HOL 阻塞）；`_request_batch()` 异步发 16KB demand → `_run_grant_listener()` 存入 `_pipe_pool[qpn]`；`run_rx()` — **concurrent dispatcher**：每帧 spawn `_rx_alloc_dma()` (8 inflight via simpy.Resource)，alloc_rx 原子槽分配 + DMA write 并行重叠 PCIe RTT；`_run_rx_completer()` 序列化 used-ring 写入（FIFO `_rx_comp_q`）；`run_tx_complete()` — VirtUsedEvent(来自 EgressFTE) → complete_tx。暴露 `register_virtio_qp()` 和 `notify_tx()` 供 driver 调用（兼容旧 API）|
| VirtDMAFetch | `rnic/dpe/txdpe/virt_dma_fetch.py` | TxDPE 子组件: **pipelined dispatcher+collector**：`_run_dispatch` 从 VirtTxCmd 取 frame_pa，`hif_port.submit()` 非阻塞发射 DMA_READ(VIRT_TX_TAG，`_MAX_INFLIGHT=32` 并发)；`_run_collect` FIFO 序收集响应，注入 PendingFetchEntry → chan_asm2fte |
...
| 模块 | 角色定位 | 核心职责 |
|------|---------|---------|
| **GS** | 发射器 (Issuer) | 全局资源守门人（**transport-agnostic**）。Select: TM Per-QP XOFF/XON。Grant: Alpha per-QP pipeline admission + GS Budget。pipeline_cost 由源头 (TE/VirtioEngine) 预计算，GS 只做统一数值比较。Dual accumulators: `qp_ready_pipeline` (normalized units) + `qp_ready_budget` (payload bytes)。模块: `gs.py`(调度) + `qc.py`(QuotaController 资源管理) |
| **SQM** | 译码器 (Decoder) | 自治 decoder。将 Raw WQEBBs 翻译为结构化 Metadata，自行分配 desc_id，无 GS 交互。Desc pool 背压提供自然流控。 |
| **vring/** | 描述符预取器 (Prefetcher) | 纯 QM 层（类比 SQM）。SplitVirtqPort: doorbell → avail_ring.idx(1 DMA) → coalesced ring[slot] read(batch×2B, 1 DMA，消除 DDR bank 串行化) → N 并行 desc_table reads(submit 后 per-entry yield 保持自然背压节奏) → 内部 `_tx_queue`。**Used ring 批量写**: complete_tx/rx 缓存 entries，满 16 或 500ns timer flush 为 coalesced DMA（2-3 writes 替代 2N writes）。**无 GS 交互**，GS admission 在 VirtioEngine 侧。 |
...
| Grant.1 | Alpha per-QP (transport-agnostic) | `max(1KB, alpha × remaining / N)` | per-QP pipeline 深度公平 | TM admission `(-4, qpn, pipeline_cost)` | pipeline_cost 很小(~148)，自然通过 |
| Grant.2 | GS Budget (`gs_budget_bytes`) | 1MB | 全局网络并发 | TxAckBridge AckNotif 处理 (`(qpn, bytes)`) | 扣减 (唯一全局 gate) |
...
**READ 自然豁免**: READ request pipeline_cost=148 (64B payload + 84B overhead) << alpha_threshold(≥1024)，自然通过 alpha check，无需特殊逻辑。READ 并发由 TE ORD 本地控制。

### GS Pipeline Admission (Transport-Agnostic)

GS 使用 transport-agnostic alpha 算法控制 per-QP pipeline 深度。**pipeline_cost 在源头 (TE/VirtioEngine) 预计算**，包含 wire overhead 和 cost_factor normalization。GS 只做统一的数值比较：

```
TE/Virtio (知道 transport 细节)          GS (transport-agnostic)
┌────────────────────────────┐          ┌─────────────────────┐
│ 计算 pipeline_cost:        │          │ Alpha per-QP:       │
│   (payload + pkts×OVERHEAD)│──demand──>│   glb_pipeline_cap  │
│   ÷ cost_factor            │          │   glb_pipeline      │
│                            │          │   qp_pipeline[qpn]  │
│ 计算 budget_bytes:         │          │                     │
│   payload (RDMA)           │──demand──>│ GS Budget:          │
│   0 (RAW/TCP/Virtio)       │          │   gs_budget         │
│                            │          │                     │
│ PacketDesc.pipeline_cost   │──pkt───> │ TM -4 return:       │
│   (per-packet, exact)      │          │   (qpn, pipe_cost)  │
└────────────────────────────┘          └─────────────────────┘
```

**pipeline_cost 公式**:
```python
# RDMA (cost_factor=1): pipeline_cost = payload + pkt_count × RDMA_PKT_OVERHEAD(84)
# Virtio (cost_factor=8): pipeline_cost = (frame_len + VIRTIO_PKT_OVERHEAD(26)) / 8
# TCP TSO (cost_factor=1): pipeline_cost = payload + pkt_count × TCP_PKT_OVERHEAD(80)
```

**Demand 格式**: `(qpn, pipeline_cost, budget_bytes, forwarded_qpc)`
- `pipeline_cost`: TE/Virtio 预计算的 normalized pipeline units
- `budget_bytes`: payload bytes for gs_budget (0 for fire-and-forget: RAW/TCP/Virtio)

**Dual accumulators**: GS 维护两个独立维度:
- `qp_ready_pipeline[qpn]`: normalized pipeline units (float)
- `qp_ready_budget[qpn]`: payload bytes for gs_budget (int)

| 参数 | 文件 | 默认值 | 说明 |
|------|------|--------|------|
| `tm_quota_bytes` | `gs/config.py` | 32KB | Global pipeline depth cap (`_glb_pipeline_cap`) |
...
- **GS transport-agnostic**: `check_and_deduct(qpn, pipeline_cost, budget_bytes)` — 无 tc, is_read, is_raw, cost_factor
- **Pipeline cost at source**: TE/VirtioEngine 知道 transport 细节（MTU, overhead, cost_factor），GS 收到的是归一化数值
...
- **Fire-and-forget budget gate bypass**: RAW/TCP/Virtio QPNs 的 budget_bytes=0，TE 通过 `_no_budget_qpns` 集合跳过 byte budget gating

### 守恒闭环

- **gs_budget**: `GS_deduct = TxAckBridge.run_bridge_ack_notif(AckNotif) → gs_credit_return_chan.put((qpn, bytes))`
- **pipeline (glb_pipeline + qp_pipeline)**: `GS_deduct(pipeline_cost) = sum(TM -4 returns per-pkt pipeline_cost)` — PacketDesc 携带精确 per-packet pipeline_cost
- **TM Per-QP XOFF/XON**: 无 credit 守恒（TM 自主监控 VOQ 字节数，静态水位）

**WRITE 4096B 守恒** (RDMA, pipeline_cost = 4096 + 5×84 = 4516):
| 事件 | gs_budget | glb_pipeline | qp_pipeline |
|------|-----------|-------------|-------------|
| Grant | -4096 | +4516 | +4516 |
| TM Admission (-4, sum of 5 pkts) | — | -4516 | -4516 |
| ACK | +4096 | — | — |

**Virtio 1500B 守恒** (pipeline_cost = (1500+26)/8 = 190.75):
| 事件 | gs_budget | glb_pipeline | qp_pipeline |
|------|-----------|-------------|-------------|
| Grant | 不扣减(budget=0) | +190.75 | +190.75 |
| TM Admission (-4) | — | -190.75 | -190.75 |

**READ 4096B 守恒** (pipeline_cost = 64+84 = 148, << alpha threshold):
| 事件 | gs_budget | glb_pipeline | qp_pipeline |
|------|-----------|-------------|-------------|
| Grant | -4096 | +148 | +148 |
| TM Admission (-4) | — | -148 | -148 |
| ACK | +4096 | — | — |

**READ_RESP 4096B 守恒** (pipeline_cost = 4096 + 5×84 = 4516):
| 事件 | gs_budget | glb_pipeline | qp_pipeline |
|------|-----------|-------------|-------------|
| RxRspBridge (-9) | 不扣减 | +4516 | +4516 |
| TM Admission (-4) | — | -4516 | -4516 |
...
- **Pipeline Credit Return at TM Admission**: TxQueueMgr.put() 在包进入 VOQ 时立即归还 `(-4, qpn, pipeline_cost)` 给 GS（从 PacketDesc.pipeline_cost 读取）
...
| 消息格式 | 来源 | 回补目标 |
|----------|------|---------|
| `(-4, qpn, pipeline_cost)` | TM TxQueueMgr.put() (at admission) | 归还 glb_pipeline + qp_pipeline（pipeline_cost 从 PacketDesc 读取，per-packet 精确值）|
| `(-6, qpn, flag)` | TM TxQueueMgr | Per-QP XOFF/XON (rate-proportional, flag: 0=XOFF, 1=XON) |
| `(-9, qpn, pipeline_cost)` | RxRspBridge.run() (READ_RESP 发送前) | 预扣减 glb_pipeline + qp_pipeline（与后续 -4 归还配对，gs_budget 不动）|
| `(qpn, byte_cnt)` | TxAckBridge.run_bridge_ack_notif (AckNotif) | gs_budget |

**已消除**: Assembly per-TC credit, `(-8, 0, flag)` Port XOFF/XON, `(-7, tc, flag)` TC XOFF/XON, `(-5, qpn, bytes)` Per-QP Quota, `(-1, bytes)` pipeline_credit, `(-3, qpn, count)` ORD, `int(bytes)` SQM refund, tc 参数（GS transport-agnostic）, cost_factor lookup（在源头完成）
...
- **READ 并发控制**: GS budget 是 READ 唯一全局 gate。TE ORD 控制 per-QP READ 并发。Alpha/XOFF 均跳过 READ
- **gs_budget 守恒**: `GS_deduct = TxAckBridge.run_bridge_ack_notif → gs_credit_return_chan.put((qpn, bytes))`
- **pipeline 守恒**: `GS_deduct(pipeline_cost) = sum(TM -4 return per-pkt pipeline_cost)` — 每个 PacketDesc 携带精确 per-packet pipeline_cost，Segmentation 创建时设置，EgressFTE 透传到 TLM_Transaction，TM 归还。WRITE/SEND: via GS grant；READ_RESP: via RxRspBridge `(-9, qpn, pipeline_cost)` 预扣减
- **GS transport-agnostic (proactive)**: `check_and_deduct(qpn, pipeline_cost, budget_bytes)` 不知道 transport type、tc、cost_factor。pipeline_cost 在源头 (TE/VirtioEngine) 预计算并 normalized
- **pipeline_cost at source**: RDMA: `payload + pkts × RDMA_PKT_OVERHEAD(84)` / cost_factor=1; Virtio: `(frame_len + 26) / 8`; TCP: `payload + pkts × TCP_PKT_OVERHEAD(80)`
...
- **GS 3 种 Parking Lot**: `_parked_on_tm_bp` (Per-QP XOFF) / `_parked_on_pipeline` (Global Alpha) / `_parked_on_budget` (GS Budget)
...
- **Virtio TX pipeline 不变量**: SplitVirtqPort 在 `_tx_depth[qpn]<64` 时持续预取（pipelined 3-step DMA）；VirtioEngine **per-QPN worker pattern**: `_run_tx_dispatch()` 路由到 per-QPN deque → `_tx_worker(qpn)` 独立处理 GS grant + pipeline pool + tx_cmd push（无跨 QPN HOL 阻塞）；batch grant 模型（"整借零还"）：本地 `_pipe_pool[qpn]` 扣减 pipeline_cost，`_LOW_WATERMARK=4096` 提前 re-request；`_run_grant_listener` 独立协程接收 ByteBudgetGrant 存入 pool 并唤醒阻塞的 worker；VirtDMAFetch pipelined dispatcher+collector（`_MAX_INFLIGHT=32`）隐藏 PCIe RTT
- **Virtio RX concurrent dispatch 不变量**: `_run_rx()` 为 dispatcher，每帧 spawn `_rx_alloc_dma()`（`_RX_MAX_INFLIGHT=8` via simpy.Resource 背压）；alloc_rx 原子槽分配（rx_last_avail 在首个 yield 前递增，SimPy 协作调度保证原子）；DMA write 通过 DMAWriteJob(done_evt) 异步完成；`_run_rx_completer` 从 `_rx_comp_q` FIFO 取出 → yield done_evt → complete_rx 序列化 used-ring 写入（Virtio entries 由 desc_idx 标识，无需匹配 avail order）
- **Virtio GS 路径**: VirtioEngine._request_batch 发送 `(qpn, 16KB_pipeline, 0, dummy_qpc)` demand → GS `_calc_demand` cap 到 `byte_budget_per_grant=16KB` → ByteBudgetGrant(pipeline_cost=16KB, byte_budget=0) → `_run_grant_listener` 存入 `_pipe_pool[qpn]`；budget_bytes=0 → 跳过 gs_budget 扣减；per-frame pipeline_cost = (frame_len + 26) / 8 在源头预计算，本地扣减
- **Fire-and-forget budget bypass**: RAW/TCP/Virtio 的 budget_bytes=0，TE 通过 `_no_budget_qpns` 集合跳过 byte budget gating（`_maybe_mark_ready`/`_pick_ready_item`/`_execute_slice` 中的 budget check）
- **Virtio used ring 批量写**: `complete_tx/complete_rx` 为普通方法（非 generator），缓存 entries 至 batch（`_USED_BATCH_MAX=16`），满 batch 或 coalesce timer（`_USED_COALESCE_NS=500` ns）到期时 flush 为 2-3 次 coalesced fire-and-forget DMA 写（elements + 单次 idx）。HIF FIFO 保证 elements 先于 idx 写入 host memory（Virtio spec ordering）。TX/RX completer 各为单 coroutine 串行调用 complete_tx/rx，无并发冲突
- **Virtio PCIe client 两层分工**: VIRT_TX_TAG(10) NP-track — VirtDMAFetch TX frame DMA_READ（`_MAX_INFLIGHT=32`）；VRING_TAG(12) NP-track — SplitVirtqPort ring metadata (avail ring, desc table, used ring, RX ring walk)。Virtio RX frame DMA 合并入 RX_EXEC_TAG(9)（RxDMAEngine 统一处理，DMAWriteJob(done_evt) 机制）。两类均竞争 PCIe tags，无 synthetic latency
...
- **READ_RESP 路由（RespBuilder → RxRspBridge）**: RespBuilder（RC）构造 `ReadRespWork` 推入 `chan_rc2me_read_resp` → ME.RxRspBridge.run() 接收后：(1) 计算 `pipeline_cost = payload + pkt_count × RDMA_PKT_OVERHEAD` 并发送 `(-9, qpn, pipeline_cost)` 到 `gs_credit_return_chan` 扣减 pipeline credit，确保后续 TM admission `(-4)` 归还时平衡；(2) 调用 `build_hdr_template()` 组装 TxCmd 推入 `chan_tx_cmd`。gs_budget 不扣减（READ response 无 ACK 归还）。RxRspBridge 是 ④ RX Respond 在 ME 的执行侧（独立于 TxEngine，职责单一）
...
python3 -m tests.st.vr_bw_net      1500 4    256K
python3 -m tests.st.vr_bw_net      64   16   64K
```
...
| `tm_quota_bytes` | `gs/config.py` | 32KB | GS global pipeline depth cap (`_glb_pipeline_cap`) |
| `port_alpha` | `gs/config.py` | 1.0 | Per-QP 公平共享比例 |
| `port_alpha_min_threshold` | `gs/config.py` | 1KB | Per-QP 最低阈值 = 1 MTU floor |
| `byte_budget_per_grant` | `gs/config.py` | 8KB | 每次 grant 的字节预算（RDMA: 4096B→2 WQE, 64B→128 WQE；Virtio batch 16KB demand 由 GS 拆为 2 grants 累积） |
...
作为 safety net 保留，主要限制 READ 并发。Pipeline 深度由 Alpha (transport-agnostic, pipeline_cost 在源头预计算) 控制，TM 负责 port/TC 调度。
